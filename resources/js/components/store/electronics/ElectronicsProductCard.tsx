    ...
    >
      ...
      >
        <img 
          ...
          alt={product.name}
          className={`w-full h-full ${isFallback ? 'object-contain p-6 bg-slate-50/50' : 'object-cover'} transition-transform duration-500 group-hover:scale-110`}
          onError={(e) => {
            setIsFallback(true);
      ...
      <div className="p-6">
        ...
        <div className="mb-4">
          {isOnSale ? (
            ...
            <span className="text-2xl font-bold text-gray-900">{format(product.price)}</span>
          )}
        </div>
