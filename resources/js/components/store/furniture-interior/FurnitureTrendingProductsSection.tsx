      ...
      <div className="container mx-auto px-6 lg:px-12">
        ...
        <div className="text-center mb-16 lg:mb-20">
          <div className="inline-flex items-center gap-3 bg-storePrimary-100 border-2 border-storePrimary-300 px-6 py-3 rounded-full mb-6 hover:scale-105 transition-transform duration-300">
            ...
            <span className="text-sm font-bold text-storePrimary-800 tracking-wider uppercase">Les plus populaires</span>
          </div>
          <h2 className="text-3xl lg:text-4xl xl:text-5xl font-bold text-slate-900 mb-8 tracking-tight">
            {content?.title || 'Tendances du moment'}
          </h2>
          <p className="text-lg lg:text-xl text-slate-700 leading-relaxed max-w-4xl mx-auto">
            {content?.description || 'Découvrez les produits les plus populaires et les tendances incontournables de cette saison.'}
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            {Array.from({ length: 8 }).map((_, index) => (
              <div key={index} className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-lg border border-yellow-200 overflow-hidden">
                <div className="aspect-square bg-gradient-to-br from-yellow-100 to-storePrimary-100 flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <p className="text-yellow-700 text-sm font-semibold">Produit tendance {index + 1}</p>
                  </div>
                </div>
                <div className="p-6">
          ...
          <div className="mt-24 bg-white/90 backdrop-blur-sm rounded-3xl p-12 lg:p-16 border-2 border-yellow-200 shadow-2xl hover:shadow-3xl transition-all duration-500">
            <div className="text-center mb-16">
              <h3 className="text-3xl lg:text-4xl font-bold text-slate-900 mb-6 tracking-tight">
                {designProcess?.title || 'Notre Engagement'}
              </h3>
              <p className="text-lg lg:text-xl text-slate-700 max-w-3xl mx-auto leading-relaxed">
                {designProcess?.description || 'De la commande à la livraison, nous vous garantissons une expérience simple, rapide et professionnelle.'}
              </p>
            </div>
            
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12">
              {(designProcess?.steps || [
                { step: '01', title: 'Sélection', description: 'Des articles choisis avec le plus grand soin' },
                { step: '02', title: 'Qualité', description: 'Des standards exigeants et certifiés' },
                { step: '03', title: 'Commande', description: 'Achat simple et sécurisé en quelques clics' },
                { step: '04', title: 'Livraison', description: 'Expédition rapide directement chez vous' }
                ...
                <div key={index} className="text-center group">
                  ...
                  <h4 className="text-lg lg:text-xl font-bold text-slate-900 mb-3 group-hover:text-storePrimary-800 transition-colors duration-300">{item.title}</h4>
