...
import { Settings as SettingsIcon, Building, DollarSign, Users, RefreshCw, Palette, BookOpen, Award, FileText, Mail, Bell, Link2, CreditCard, Calendar, HardDrive, Shield, Bot, Cookie, Search, Webhook, Wallet, MessageSquare, Phone } from 'lucide-react';
...
export default function Settings() {
  ...
  const { systemSettings = {}, cacheSize = '0.00', timezones = {}, dateFormats = {}, timeFormats = {}, paymentSettings = {}, whatsappVariables = {}, telegramVariables = {}, webhooks = [], availableModules = {}, notificationTemplates = [], auth = {}, flash } = usePage().props as any;
  ...
  const sidebarNavItems = allSidebarNavItems.filter(item => {
    // Exclude webhook settings for SuperAdmin
    if (item.permission === 'manage-webhook-settings' && auth.user?.type === 'superadmin') {
      return false;
    }
    ...
    if (auth.user && auth.user.type === 'company') {
      // Only allow system settings, email settings, brand settings, currency settings, payment settings, webhook settings, twilio settings, and settings
      return ['manage-system-settings', 'manage-email-settings', 'manage-brand-settings', 'manage-currency-settings', 'manage-payment-settings', 'manage-webhook-settings', 'manage-email-notification-settings', 'manage-twilio-settings'].includes(item.permission);
    }
    return false;
  });
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (scrollPosition >= twilioSettingsPosition) {
        ...
        <div className="flex-1">
          ...
          {auth.user?.type === 'company' && (
            <section id="twilio-settings" ref={twilioSettingsRef} className="mb-8">
              <TwilioSettings systemSettings={systemSettings} templates={notificationTemplates || []} />
            </section>
          )}
