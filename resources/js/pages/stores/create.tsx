...
function ThemePreviewCard({ theme, isSelected, onSelect }: { theme: any; isSelected: boolean; onSelect: () => void }) {
  ...
  return (
    <div 
      ...
      onClick={onSelect}
    >
      <div className="relative aspect-video overflow-hidden rounded-md bg-gray-100 dark:bg-gray-800 theme-preview-container">
        {!loaded && (
        ...
        <img
          ...
          className={`h-full w-full object-cover theme-preview-image transition-opacity duration-300 ${loaded ? 'opacity-100' : 'opacity-0'}`}
          onError={(e) => {
            setLoaded(true);
